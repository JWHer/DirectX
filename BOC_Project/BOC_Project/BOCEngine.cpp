...
bool BOCEngine::init() {
	...
	ID3DBlob* pVSBlob = NULL;
	hr = CompileShaderFromFile(L"FX/shader.fx", "VS", "vs_5_0", &pVSBlob);
	if (FAILED(hr))
	{
		MessageBox(NULL,
	...
	// Compile the pixel shader
	ID3DBlob* pPSBlob = NULL;
	hr = CompileShaderFromFile(L"FX/shader.fx", "PS", "ps_5_0", &pPSBlob);
	if (FAILED(hr))
	{
		MessageBox(NULL,
			L"The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", L"Error", MB_OK);
	...
	// Compile the pixel shader
	pPSBlob = NULL;
	hr = CompileShaderFromFile(L"FX/shader.fx", "PSSolid", "ps_5_0", &pPSBlob);
	if (FAILED(hr))
	{
		MessageBox(NULL,
			L"The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", L"Error", MB_OK);
	...
	wireframeDesc.FillMode = D3D11_FILL_SOLID;//wire
...
void BOCEngine::DrawScene() {
	...
	for (int m = 0; m < 2; m++)
	{
		...
		mImmediateContext->PSSetShader(mPixelShaderSolid, NULL, 0);
		mImmediateContext->DrawIndexed(box.iCount, 0, 0);
	}
